...
class VideoTest(object):
    ...
    def run(self, video_path = 0, start_frame = 0, conf_thresh = 0.6):
        ...
        # PiCameraからのリアルタイム動画の場合、ここに「while True」を置かないと画像がほとんど変わらない。
        # その場合、vid.readの前にある「while True」はコメント化すること。
        # while True:
        ...
        while True:
            ...
            if key == ord('i'):
                ...
                print("検出開始: " + datetime.now().strftime("%H:%M:%S"))

                y = self.model.predict(x)

                print("検出終了: " + datetime.now().strftime("%H:%M:%S"))
